...
static int parse_sleep_products_config(SleepOperation operation, char ***_blacklist, char ***_whitelist) {
        ...
        if (!f)
                ...
                r = config_parse
                        (NULL, PKGSYSCONFDIR "/sleep-products.conf", f, "CanSuspend\0CanHibernate\0CanHybridSleep\0",
                                 config_item_table_lookup, (void*) items, 0, NULL, NULL);
                if (r < 0)
...
static int can_sleep_internal(
        ...
        if (parse_sleep_products_config(operation, &blacklist, &whitelist) < 0)
                return false;
